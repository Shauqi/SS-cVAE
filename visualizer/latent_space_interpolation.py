...
def draw_latent_features_with_class(tsne_feature_space, label, output_dir, fig_name, dot_size = 5, pos_index_1 = 0, neg_index_1 = 0, pos_index_2 = 0, neg_index_2 = 0, arrow_head_width = 0.1, arrow_head_length = 0.05):
    ...
    plt.scatter(neg_features[:,0], neg_features[:,1], c = 'b', s = dot_size, label = 'neg')
    plt.scatter(pos_features[:,0], pos_features[:,1], c = 'r', s = dot_size, label = 'pos')
    plt.arrow(tsne_feature_space[pos_index_1, 0], tsne_feature_space[pos_index_1, 1], pos_to_neg_interpolated_features[1, 0] - tsne_feature_space[pos_index_1, 0], pos_to_neg_interpolated_features[1, 1] - tsne_feature_space[pos_index_1, 1], color='purple', length_includes_head=True, head_width=arrow_head_width, head_length=arrow_head_length)
    for i in range(1, pos_to_neg_interpolated_features.shape[0] - 2):
        plt.arrow(pos_to_neg_interpolated_features[i, 0], pos_to_neg_interpolated_features[i, 1], pos_to_neg_interpolated_features[i+1, 0] - pos_to_neg_interpolated_features[i, 0], pos_to_neg_interpolated_features[i+1, 1] - pos_to_neg_interpolated_features[i, 1], color='purple', length_includes_head=True, head_width=arrow_head_width, head_length=arrow_head_length)
    plt.arrow(pos_to_neg_interpolated_features[-2, 0], pos_to_neg_interpolated_features[-2, 1], tsne_feature_space[neg_index_1, 0] - pos_to_neg_interpolated_features[-2, 0], tsne_feature_space[neg_index_1, 1] - pos_to_neg_interpolated_features[-2, 1], color='purple', length_includes_head=True, head_width=arrow_head_width, head_length=arrow_head_length)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            ...
            z_mu, z_log_var, s_mu, s_log_var = cvae_model.test_encode(batch, device)
            Z_MU_arr += z_mu
            S_MU_arr += s_mu
    ...
    pos_index_1 = np.where(label_arr == 1)[0][32] # 22, 3, 7, 10, 16, 26
    neg_index_1 = np.where(label_arr == 0)[0][8] # 5

    neg_index_2 = np.where(label_arr == 0)[0][7] #
    pos_index_2 = np.where(label_arr == 1)[0][33] # 22, 3, 7, 10, 16, 26, 31
    ...
    pos_to_neg_interpolated_latents = interpolate_latent(neg_s_mu_1, pos_s_mu_1, n_steps = 4)
    pos_to_neg_background_interpolated_latents = interpolate_latent(neg_z_mu_1, pos_z_mu_1, n_steps = 4)
    ...
    # draw_interpolated_latent_with_cvae(cvae_model, pos_to_neg_interpolated_latents, pos_to_neg_background_interpolated_latents, orig_pos_image_1, orig_neg_image_1, output_dir, "interpolated_latent_cvae_pos_to_neg")

    # draw_interpolated_latent_with_cvae(cvae_model, neg_to_pos_interpolated_latents, neg_to_pos_background_interpolated_latents, orig_neg_image_2, orig_pos_image_2, output_dir, "interpolated_latent_cvae_neg_to_pos")

    draw_interpolated_latent_with_cvae(cvae_model, pos_to_neg_interpolated_latents, pos_to_neg_background_interpolated_latents, orig_neg_image_1, orig_pos_image_1, output_dir, "interpolated_latent_cvae_pos_to_neg")

    # draw_interpolated_latent_with_gan(gan_model, pos_to_neg_interpolated_latents, pos_to_neg_background_interpolated_latents, orig_pos_image_1, orig_neg_image_1, output_dir, "interpolated_latent_gan_pos_to_neg")

    # draw_interpolated_latent_with_gan(gan_model, neg_to_pos_interpolated_latents, neg_to_pos_background_interpolated_latents, orig_neg_image_2, orig_pos_image_2, output_dir, "interpolated_latent_gan_neg_to_pos")
