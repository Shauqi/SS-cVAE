...
def filter_patches(config, mode = 'Train'):
    INPUT_DIR = config['TIL_VS_OTHER_PATCH_PREPROCESSING']['PATCH_FILTER']['INPUT_DIR']
    OUTPUT_DIR = config['TIL_VS_OTHER_PATCH_PREPROCESSING']['PATCH_FILTER']['OUTPUT_DIR']
    VISUALIZATION_DIR = config['TIL_VS_OTHER_PATCH_PREPROCESSING']['PATCH_FILTER']['VISUALIZATION_DIR']
    data_dir = f'{INPUT_DIR}/{mode}'
    img_files = glob.glob(f'{data_dir}/*')
    output_dir = os.path.join(OUTPUT_DIR,'til_vs_other_filtered', mode)
...
def combine_filtered_patches(config, mode = 'Train'):
    INPUT_DIR = config['TIL_VS_OTHER_PATCH_PREPROCESSING']['PATCH_FILTER']['OUTPUT_DIR']
    OUTPUT_DIR = config['TIL_VS_OTHER_PATCH_PREPROCESSING']['PATCH_FILTER']['OUTPUT_DIR']
    input_dir = os.path.join(INPUT_DIR, 'til_vs_other_filtered', mode)
    img_files = glob.glob(f'{input_dir}/*')
    combined_output_dir = os.path.join(OUTPUT_DIR, 'til_vs_other_combined', mode)
