...
class Visualizer():
    def __init__(self, model, generator, dataset, model_dir, z_dist, dvae_latent_dim = 20,
                 save_images=True,
                 loss_of_interest=None,
                 display_loss_per_dim=False,
                 max_traversal=0.475,  # corresponds to ~2 for standard normal
                 upsample_factor=1, img_size = 64):
        """
        ...
        self.latent_dim = dvae_latent_dim #self.model.latent_dim
    ...
    def _traverse_line(self, idx, n_samples, data=None):
        ...
        if data is None:
            ...
            with torch.no_grad():
                samples, post_mean, post_logvar = self.model(data.to(self.device), encode_only=True)
    ...
    def _save_or_return(self, to_plot, size, filename, is_force_return=False):
        ...
        kwargs = dict(nrow=size[1], pad_value=(1 - 0)) #get_background(self.dataset)))
    ...
    def _decode_latents(self, latent_samples):
        ...
        return self.model(c = latent_samples, decode_only = True).data.cpu()
        # return self.model.decoder(latent_samples).cpu()

    def _generate_latents(self, latent_samples, z):
        latent_samples = latent_samples.to(self.device)
        # z = torch.squeeze(z)
        print(z.shape)
        z = z.repeat(latent_samples.shape[0], 1)
        print(latent_samples.shape, z.shape)
        z_ = torch.cat([z, latent_samples], 1)
        return self.generator(z_).data.cpu()
    ...
    def reconstruct(self, data, z, size=(8, 8), is_original=True, is_force_return=False):
        ...
        with torch.no_grad():
            ...
            z = z[:n_samples, ...]
            _, c, mu, logvar = self.model(originals, test = True)
            z_ = torch.cat([z, mu], 1)
            recs = self.generator(z_)

        originals = originals.cpu()
        recs = recs.view(-1, *self.img_size).cpu()
    ...
    def traversals(self, data=None, z = None, is_reorder_latents=False,
                   n_per_latent=8, n_latents=None, is_force_return=False):
        ...
        decoded_traversal = self._generate_latents(torch.cat(latent_samples, dim=0), z)
    ...
    def reconstruct_traverse(self, data, z,
                             is_posterior=True,
                             n_per_latent=8,
                             n_latents=None,
                             is_show_text=False):
        ...
        if is_show_text:
            ...
            labels = ['orig', 'recon'] + ["Z_{}".format(l) for l in range(n_latents)] #+ ["KL={:.4f}".format(l) for l in losses]
